document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateLabelColors(layerName) {
    const isDarkBackground =
      layerName === "googleHybrid" || layerName === "googleSat";
  ...
  const drawingStyles = {
    ...
    measurement: {
      // Special style for measurement lines
  ...
  projectDetailPopup.style.transition =
    "opacity 0.3s ease, transform 0.3s ease"; // Add this line
  ...
  const apiUrl = "/gis/projects/locations";
    ...
    .then((response) => {
      if (!response.ok) {
        return response
          ...
          .then((errData) => {
            throw new Error(
              `HTTP error! Status: ${response.status}. Details: ${JSON.stringify(errData)}`,
            );
          })
          .catch(() => {
            throw new Error(
  ...
  function createProjectMarker(project) {
    ...
    if (isNaN(lat) || isNaN(lng)) {
      console.warn(
        `Skipping project "${project.project_name}" due to invalid coordinates: lat=${project.lat}, lng=${project.lng}`,
      );
      return;
    }
    ...
    const markerHtml = `
        <div class="project-marker-content">
            ...
            ${
              project.project_name
                ? `<span class="project-label-text">${project.project_name
                    .split(" ")
                    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
                    .join(" ")}</span>`
                : ""
    ...
    let rawType = project.project_type
      ? project.project_type.toLowerCase().trim()
      : "other";
  ...
  function updateLabelVisibility() {
    ...
    const shouldShowLabels =
      currentZoom >= zoomThresholdForLabels && activeFilters.showNames;
    ...
    allMarkers.forEach((item) => {
      ...
      if (map.hasLayer(marker)) {
        ...
        if (
          markerElement &&
          markerElement.classList.contains("label-visible")
        ) {
          markerElement.classList.remove("label-visible");
        }
      }
      ...
      countySelect.addEventListener("change", function () {
        ...
        if (this.value !== "all" && countyLayer) {
          ...
          countyLayer.eachLayer((layer) => {
            const countyName =
              layer.feature.properties.county_name.toLowerCase();
  ...
  function setupFilterEventListeners() {
    ...
    document
      ...
      .forEach((checkbox) => {
        checkbox.addEventListener("change", function () {
          ...
          const subOptions = document.querySelectorAll(
            `.sub-option[data-main="${optionId}"]`,
          );
    ...
    document.querySelectorAll(".sub-option").forEach((checkbox) => {
      checkbox.addEventListener("change", function () {
        ...
        const relatedSubOptions = document.querySelectorAll(
          `.sub-option[data-main="${mainOptionId}"]`,
        );

        // If any suboption is checked, ensure the parent checkbox is also checked
        const anySubOptionChecked = Array.from(relatedSubOptions).some(
          (opt) => opt.checked,
        );
    ...
    allMarkers.forEach((item) => {
      ...
      const countyMatch =
        ...
        county.toLowerCase().trim() ===
          activeFilters.county.toLowerCase().trim();

      // Check if this marker should be excluded based on specific status+type combination
      const isExcluded = activeFilters.exclusions.some(
        (exclusion) => exclusion.status === status && exclusion.type === type,
      );
  ...
  function showLayerPopup() {
    ...
    if (layerPopup) {
      ...
      requestAnimationFrame(() => {
        // Ensure display:block is applied first
  ...
  function toggleSubOptions(option) {
    ...
    if (subOptions && icon) {
      ...
      console.error(
        "Sub-options or icon element not found for option:",
        option,
      );
    }
  }
  ...
  function setupDrawingToolButtons() {
    ...
    optionIcons.forEach((icon, index) => {
      icon.addEventListener("click", function () {
        ...
        document.getElementById("export_features").style.display =
          "inline-block";
      });
    });
  }
