  ...
  router.get("/gis/projects/:id/article", async (req, res) => {
    ...
    try {
      const projectRows = await withDb((client) =>
        client.query(
          `SELECT p.id, p.project_name, p.description, p.people_served, p.progress,
                  c.county_name AS county, s.status, t.type AS project_type,
                  pa.id AS article_id, pa.hero_image_url, pa.published_at, pa.updated_at
           FROM public.project p
           JOIN public.county c ON p.county_id = c.id
           JOIN public.status s ON p.project_status = s.id
           JOIN public.type t ON p.project_type = t.id
      ...
      const blocks = await withDb((client) =>
        client.query(
          `SELECT id, block_type, content, sort_order
           ...
           WHERE article_id = $1 ORDER BY sort_order ASC`,
          [project.article_id],
        ),
      );
  ...
  router.post("/gis/projects/:id/article", requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (existing.length > 0)
        return res
          .status(409)
          .json({ error: "Article already exists. Use PUT to update." });
      const articleRows = await withDb((client) =>
        client.query(
          `INSERT INTO public.project_article (project_id, hero_image_url) VALUES ($1, $2) RETURNING id`,
      ...
      if (Array.isArray(blocks) && blocks.length > 0) {
        for (const block of blocks) {
          ...
          await withDb((client) =>
            client.query(
              `INSERT INTO public.project_content_block (article_id, block_type, content, sort_order) VALUES ($1, $2, $3, $4)`,
              [
                articleId,
                block.block_type,
                block.content,
      ...
      await withDb((client) =>
        client.query(
          ...
          [
            req.session.user.id,
            "ADD_ARTICLE",
            `Added article for project ID: ${projectId}`,
          ],
        ),
      );
      res.status(201).json({
