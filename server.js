...
app.get('/gis/logout', (req, res) => {
    req.session.destroy((err) => {
        ...
        res.redirect('/gis');:%s/\/reset-password/\/gis\/reset-password/g
    });
});
...
app.delete('/gis/project/:id', requireAuth, superAdminAuth, async (req, res) => {
    const projectId = parseInt(req.params.id, 10);
     if (isNaN(projectId)) {
        return res.status(400).json({ error: 'Invalid project ID format.' });
    }
...
app.post('/gis/admins/:id/reset-password', requireAuth, superAdminAuth, async (req, res) => {
    let client;
    try {
        client = await pool.connect();
        const { id } = req.params;
